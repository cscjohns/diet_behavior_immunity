...
emma_diet <- data.frame(t(parApply(clus, r_matrix, 1, function(y){
  library(EMMREML)
  emma <- emmreml(y = y,  # model each gene
                  X = model.matrix(~ sample_info$diet),  # design matrix
...
colnames(emma_diet) <- c("es_diet", "diet_pval", "diet_var")  # rename columns

# Calculate standardized betas
emma_diet$std_beta_diet <- emma_diet$es_diet / sqrt(emma_diet$diet_var)

# Gene expression (GE) ~ DAB----------------------------------------------------
# Model gene expression as a function of DAB using linear mixed effects model.
...
emma_DAB <- data.frame(t(parApply(clus, r_matrix, 1, function(y){
  library(EMMREML)
  emma <- emmreml(y = y,  # model each gene
...
colnames(emma_DAB) <- c("es_DAB", "DAB_pval", "DAB_var")  # rename columns

# Calculate standardized betas
emma_DAB$std_beta_DAB <- emma_DAB$es_DAB / sqrt(emma_DAB$DAB_var)

# Empirical False Discovery Rate (FDR)------------------------------------------

# Model the effect of diet on gene expression in 1000 permutations, permuting
# the meta data in order to determine a null distribtuion of p-values. Then, use
# the permuted null distribution to calculate an empirical FDR. All of this will
# be completed on the high performance computing cluster.

save(emma_DAB, emma_diet, kinship, r_matrix, sample_info,
     file = "data_files/pval_perms_in.Rdata")

# scp pval_perms_in.Rdata to directory in cluster, then run the following:
# dbi_4_pval_distribution.slurm (runs 100 iterations of dbi_4_perm_pvals.R)
# dbi_5_pval_DAB_distribution.slurm (runs 100 iterations of dbi_5_perm_DAB_pvals.R)
# dbi_6_perm_collation.R (collates the outputs of the other two scripts to create two
#  data frames containing null p-value distributions, which are then used to
#  calculate an empirical FDR)

load("data_files/emma_diet_DAB_out.Rdata")

# Create data frame combining diet and DAB models
emma_diet$stable_ID <- row.names(emma_diet)
emma_diet <- as_tibble(emma_diet) %>%
  arrange(stable_ID)
emma_DAB$stable_ID <- row.names(emma_DAB)
DEG_info <- left_join(emma_diet, emma_DAB, by = "stable_ID")
...
mfas_genes <- getBM(attributes = c("ensembl_gene_id", "external_gene_name"),
                    ...
                    values = unlist(DEG_info$stable_ID),
                    mart = mart)
