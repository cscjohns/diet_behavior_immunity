...
for (i in 1:nReps) {
  ...
  perm_meta <- sample_info[perm_ids, "diet"]
  ...
  lmm_perm <- parApply(clus, r_matrix, 1, function(y){
    library(EMMREML)
    emma <- emmreml(y = y,
                    X = as.matrix(model.matrix(~ perm_meta$diet)),
...
save(perm_pvals, file = paste0("/gscratch/csde/smacklab/corbin/pval_distribution/pvals_", in_args[1], "_out.Rdata"))
